...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry)
{
    ...
    entry.push_back(Pair("tx-comment", wtx.strMERCURYSpeech));
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 4 && params[4].type() != null_type && !params[4].get_str().empty())
        strMERCURYSpeech = params[4].get_str();
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, strMERCURYSpeech);
    if (!fCreated)
    {
