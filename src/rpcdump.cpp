...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <mercuryprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <mercuryaddress>\n"
            "Reveals the private key corresponding to <mercuryaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Mercury %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    // can be added into your mercury wallet. 
    ...
    if (nImported)
        if (fRescan)
        {
            LogPrintf("Searching last %i blocks (from block %i) for dug Mercurys...\n", pindexBest->nHeight - pindexRescan->nHeight, pindexRescan->nHeight);
