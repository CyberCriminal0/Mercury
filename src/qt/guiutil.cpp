...
{
    // return if URI is not valid or is no mercury: URI
    if(!uri.isValid() || uri.scheme() != QString("mercury"))
        return false;
...
{
    // Convert mercury:// to mercury:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("mercury://"))
    {
        uri.replace(0, 12, "mercury:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Mercury.lnk";
}
...
{
    return GetAutostartDir() / "mercury.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Mercury-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  mercury-qt [" + tr("command-line options") + "]                     " + "\n";
