...
Clone the git repositories for mercury and gitian and then checkout the mercury version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nochowderforyou/mercury
cd mercury
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mercury repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the mercury repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mercury/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nochowderforyou/mercury
...
./bin/gbuild --commit mercury=${COMMIT} --url mercury=${URL} ../mercury/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mercury=${COMMIT} --url mercury=${URL} ../mercury/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mercury=${COMMIT} --url mercury=${URL} ../mercury/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mercury-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mercury-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mercury-build.assert
```
...
[mercury/gitian.sigs](https://github.com/mercury/gitian.sigs/) repository, or if that's not possible create a pull
